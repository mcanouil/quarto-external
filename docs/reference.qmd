...
The extension reads `true`, `1`, and `yes` as true, and `false`, `0`, and `no` as false.
Any other value is read as if the attribute had not been written.

The schema accepts only `true` and `false`, so the four extra spellings are reported even though they work, and an unrecognised value is reported as well.
Write `true` or `false` to keep the render quiet.
...
| The file can be read or fetched. | Stops the render. |
...
A failure over the shift includes the content anyway, with no shift or a truncated one.
Every other failure in that table leaves the surrounding document intact and removes only the shortcode, except one.

A target that cannot be read or fetched stops the render.
Pandoc raises rather than reporting the failure, so the extension never reaches the message it holds for that case.
Check a path before you rely on it, and expect a failed build rather than a document with a gap in it.
