...
The schema accepts only `true` and `false`, so every other spelling is reported even though the extension honours it.
...
Every failure in that table leaves the surrounding document intact and removes only the shortcode, except one.
